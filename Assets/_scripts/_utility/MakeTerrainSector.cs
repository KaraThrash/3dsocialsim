...
public class MakeTerrainSector : MonoBehaviour
{
            ...
            while (xpos < length)
            {
                ...
                if (Physics.Raycast(squareParent.position + new Vector3(xpos, 5, zpos), Vector3.down, out hit, 16.0f))
                {
                    if (Vector3.Angle(hit.normal, Vector3.up) < 5)
                    {
                        ...
                        {
                            // Instantiate(cornerfence,
                            // hit.point, squareParent.rotation);
                        }
    ...
    public void LoadMapFromBlueprint()
    {
        ...
        // } else if
        // (tempstring[i].Equals("w"))
        // {
        // clone.GetComponent<MeshRenderer>().material
        // =
        // watermat;
        // clone.GetComponent<TerrainSquare>().terrainStatus
        // = "water";

        // } else if
        // (tempstring[i].Equals("h"))
        // {
        // clone.GetComponent<MeshRenderer>().material
        // =
        // fencemat;
        ...
        // "fence";
        // clone =
        // Instantiate(hortfence,
        // new
        // Vector3(squareParent.position.x
        // + i,
        // squareParent.position.y,
        // squareParent.position.z
        // - count),
        // squareParent.rotation);
